...
- Name: app.spec.ts >> tasks: empty state renders
- Location: tests/app.spec.ts:66:5
...
Locator: getByTestId('empty-tasks').or(getByTestId('task-list'))
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByTestId('empty-tasks').or(getByTestId('task-list'))
...
> 73  |   ).toBeVisible()
  ...
  125 |   if (page.url().includes("/login")) {
  126 |     test.skip(true, "Not authenticated")
  127 |   }
  128 |   await expect(page.getByTestId("settings-form")).toBeVisible()
  129 |   await expect(page.getByTestId("btn-save-settings")).toBeVisible()
  130 |   await expect(page.getByTestId("btn-add-child")).toBeVisible()
  131 |   await expect(page.getByTestId("member-list")).toBeVisible()
  132 | })
  133 | 
```